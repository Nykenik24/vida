...
vida2d_status_t vida2d_draw_point(vida2D_t *ctx, vida2d_vec2_t pos) {
  return V2D_BOOL_TO_STATUS(SDL_RenderPoint(ctx->renderer, pos.x, pos.y));
}

vida2d_status_t vida2d_draw_line(vida2D_t *ctx, vida2d_vec2_t start,
                                 vida2d_vec2_t end) {
  return V2D_BOOL_TO_STATUS(
      SDL_RenderLine(ctx->renderer, start.x, start.y, end.x, end.y));
}
...
vida2d_status_t vida2d_draw_triangle(vida2D_t *ctx, vida2d_draw_mode_t mode,
  ...
  case V2D_DRAW_FILLED: {
    ...
    return V2D_BOOL_TO_STATUS(
        SDL_RenderGeometry(ctx->renderer, NULL, verts, 3, NULL, 0));
  }
  }
}
...
vida2d_status_t vida2d_set_color(vida2D_t *ctx, vida2d_color_t color) {
  return V2D_BOOL_TO_STATUS(SDL_SetRenderDrawColor(ctx->renderer, color.r,
